...
// all files ending with `.spec.js(x)` and trace its path.
// By passing in true, we permit this process to occur recursively.
const sourcePath = require('./config').paths.source;
const context = require.context(sourcePath, true, /\.spec\.(js|jsx)/);
